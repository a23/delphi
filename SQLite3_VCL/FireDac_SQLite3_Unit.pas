...
uses
  ...
  FireDAC.Stan.ExprFuncs, FireDAC.Phys.SQLite,System.IOUtils, FireDAC.Phys.PG;
...
type
  TForm1 = class(TForm)
    ...
    Edit_SetVal: TEdit;
    Panel2: TPanel;
    SpeedButton_FireDAC_BLOB: TSpeedButton;
    SpeedButton_dbExprss_BLOB: TSpeedButton;
    Image1: TImage;
...
//  if FDConnection1.Connected=True  then Label1.Caption:='FireDAC Connect'
//  else
...
begin
  ...
  //SQLConnection101.Params.Values['Database'] := GetCurrentDir+'\'+LabeledEdit2.Text; // 'sq3_sales.s3db');   //System.IOUtils

  //SQLConnection101.Params.Values['Database']:='E:\SQLite3\sq3.s3db';
  // SQLConnection101.Params.Add('Database=E:\SQLite3\sq3.s3db');

  //SQLConnection101.Params.Values['OpenMode']:=ComboBox1.Text;
   //SQLConnection101.Params.Add('Database=D:\SQLite3\Win32\Debug\sq3.s3db');

//  SQLConnection101.VendorLib    :=GetCurrentDir+'\'+LabeledEdit6.Text; //ベンダーDLL
  ...
  Edit_TSNAME100.Text:='百①Ⅱ㈱十構蚕柿港伝';
...
begin
  ...
  //  ShowMessage('こみっと　しました！');
  //except
  //  FDConnection1.Rollback;
  //  ShowMessage('えらーロールバックしました。');
  //end;  //----------------------------------------------------------------------------
  //-- シーケンス管理用テーブル作成
  //----------------------------------------------------------------------------
  FDConnection1.StartTransaction;
  try
    try
      SQLstmt:= 'DROP TABLE IF EXISTS TEST_NO_1' ;
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      FDQuery1.ExecSQL;
    finally
    ...
    //---
    try
      SQLstmt:='create table TEST_NO_1 (seq_id INTEGER PRIMARY KEY AUTOINCREMENT, di integer)';
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      //FDQuery1.ParamByName('i1').AsInteger:=1;
      FDQuery1.ExecSQL;
    finally
      FDQuery1.Close;
    end;
    ...
    //----------------------------------------------------------------------------
    try
      SQLstmt:= 'DROP TABLE IF EXISTS TEST_NO_2' ;
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      FDQuery1.ExecSQL;
    finally
    ...
    //---
    try
      SQLstmt:='create table TEST_NO_2 (seq_id INTEGER PRIMARY KEY AUTOINCREMENT, di integer)';
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      //FDQuery1.ParamByName('i1').AsInteger:=1;
      FDQuery1.ExecSQL;
...
begin
  ...
  try
    //----------------------------------------------------------------------------
    try
      SQLstmt:= 'insert into TEST_NO_1 (di) values (1)' ;
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      FDQuery1.ExecSQL;
    finally
    ...
    //---
    try
      SQLstmt:='select seq_id from TEST_NO_1 order by seq_id desc';
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      //FDQuery1.ParamByName('i1').AsInteger:=1;
      FDQuery1.Open;
      if FDQuery1.Eof then  Edit_NextVal.Text:='0'
      else
        Edit_NextVal.Text:=IntToStr(FDQuery1.FieldByName('seq_id').AsInteger);
    finally
      FDQuery1.Close;
    end;
    //-----------------------------------------------END
...
begin
  ...
  try
    ...
    //----------------------------------------------------------------------------
    if (Edit_SetVal.Text<'1') then  Edit_SetVal.Text:='1';

    wSetVal:= IntToStr(StrToInt(Edit_SetVal.Text)-1);
    try
      SQLstmt:= 'update sqlite_sequence set seq = '+ wSetVal
            +' where name = '+s+'TEST_NO_1'+s;
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text :=SQLstmt;
      FDQuery1.ExecSQL;
    finally
...
Label_RDB.Caption:='TEST_TBL';
  ...
  try
    SQLstmt :='select tbl_name from sqlite_master where type='
              +s+'table'+s+' and name=:TABLEnm';
      FDQuery1.SQL.Clear;
      FDQuery1.SQL.Text:= sqlstmt;
      ...
      begin
        ...
        SpeedButton4.Caption  :='NonDB';
      end
      else
      begin
        Label_RDB_CHK.Caption :=Trim(FDQuery1.FieldByName('tbl_name').AsString);
        SpeedButton4.Caption  :=Trim(FDQuery1.FieldByName('tbl_name').AsString);
      end;
  finally
      FDQuery1.Close;
      //FDQuery1.Free
